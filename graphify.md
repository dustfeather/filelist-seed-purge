...
```sh
cd "/home/dustfeather/projects/filelist-seeder"
graphify update .                                    # rebuild this graph (AST, no LLM)
graphify global add graphify-out/graph.json --as filelist-seeder  # merge into the federation
```

To re-extract with the LLM backend (richer, slower):

```sh
vault-graphify.sh --repo filelist-seeder
```

Once you `git init` here, run `vault-graphify.sh --repo filelist-seeder` once and the
